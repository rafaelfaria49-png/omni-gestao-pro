 ...
 *
 * Estado atual (GOAL 020 · 144 · CONTAINMENT OFF da janela de diagnóstico browser-assisted
 * sincronizado): **DORMENTE** — `activationId`, `notBeforeUtc` e `expiresAtUtc` restaurados a
 * `null`. A janela `wsdl-h9h10-20260902-0430z-772103b09d9477ca` (02/09 04:30→04:40Z)
 * **EXPIROU SEM CONSUMO**: a invocation administrativa NÃO foi realizada e nenhum GET WSDL
 * foi disparado. Contadores LOCAIS desta janela: `WSDL_ADMIN_CALL_COUNT=0`,
 * `WSDL_EXTERNAL_GET_COUNT=0` (o histórico acumulado do GOAL 020 NÃO é zero — a execução 137
 * registrou 1 chamada administrativa HTTP 200 e batch dos 6 alvos). A activation NÃO foi
 * consumida (one-shot íntegro, morta por relógio). É evidência histórica; não é configuração
 * executável e jamais deve ser re-materializada sem NOVA autorização humana (a autorização
 * desta janela NÃO é reutilizável). As activations de 30/08 (14:40z `fed207ff`, 20:05z
 * `51354088`) e as de 31/08 (`0c42c4389f65469d` 03:00z, `99c21bca85a94cef` 19:00z,
 * `891f55e242004bd2` 22:30z, todas expiradas sem consumo) seguem históricas/proibidas. Gate 2
