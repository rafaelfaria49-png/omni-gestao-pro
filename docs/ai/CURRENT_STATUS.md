...
Auditoria [`CONTADOR_HUB_FISCAL_PASSO0_AUDIT_018.md`](../contador/CONTADOR_HUB_FISCAL_PASSO0_AUDIT_018.md) na main (PR **#88** merge `fac38130`). ADR-CONTADOR-007 **Accepted** com predicado Opção A, cancelados fora de `05-XML`, competência por `dhEmi` de `xmlAutorizado` **sem fallback**, flag `CONTADOR_FISCAL_READER` default off + allowlist (não `fiscalEnabled`), homologação isolada (nunca Production) e **proibição** de reusar `fiscalXmlReader.readAuthorizedDocument` as-is. Preparação: [`CONTADOR_HUB_FISCAL_HOMOLOGATION_PREP_018.md`](../contador/CONTADOR_HUB_FISCAL_HOMOLOGATION_PREP_018.md). Runtime vivo **não** validável. **GOAL 018 não aberto / não implementado.** Trilha AEP `contador` **PAUSED**.
...
- [`CONTADOR_HUB_ADRS_PROPOSTOS_001.md`](../contador/CONTADOR_HUB_ADRS_PROPOSTOS_001.md) — 8 ADRs (001/003/004/005/006 Accepted G2; 002/008 Accepted G3; **007 Accepted 2026-08-19**; GOAL 018 não aberto).
...
**Próximo:** GOAL 012 já entregue (fechamento com snapshot). Gates seguintes: **G3** realizado (portal v2 + ADRs 002/008); **G4** antes do GOAL 019 (retirada do legado). **ADR-007 Accepted** — GOAL 018 **não autorizado**.
