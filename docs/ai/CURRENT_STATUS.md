...
## Contador HUB — GOAL 018 implementação read-only HOMOLOGACAO · 20/08/2026
...
Reader A em `lib/contador/readers/fiscal.ts` (SELECT `NotaFiscal` isolado por `storeId`; nunca `fiscalXmlReader.readAuthorizedDocument`; nunca grava FiscalLog/EventoFiscal). Checklist atualiza só o sinal fiscal. Pacote substitui `05-XML/LEIA-ME.md` apenas com XML entregável (texto UTF-8 persistido + sha256). UI: relatório fiscal mínimo somente leitura. Runtime homolog local (Postgres nativo 5432 nesta prova): 1 AUTORIZADA/HOMOLOGACAO entregável, 1 rejeitada, 1 cancelada; FiscalLog=0; EventoFiscal=0. **Não mergear. Não `track close`.** Revisão independente pendente. Detalhe: [`CONTADOR_HUB_FISCAL_HOMOLOGATION_PROVISION_018.md`](../contador/CONTADOR_HUB_FISCAL_HOMOLOGATION_PROVISION_018.md).
