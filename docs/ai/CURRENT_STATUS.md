...
> **Adendo — 06 Set 2026 · Fiscal · GOAL 021 observabilidade fiscal consolidada e bateria ampla de cenários offline (`FISCAL-OBSERVABILITY-SCENARIO-BATTERY-021`):** implementação do serviço consolidado de observabilidade fiscal read-only (`lib/fiscal/observability`), expondo telemetria estruturada das 4 dimensões canônicas (fila, estado incerto/reconciliação, throttling cStat 656 e contingência offline). Superfície interna protegida via endpoint `GET /api/internal/fiscal/observability` com autenticação em tempo constante via `FISCAL_QUEUE_INTERNAL_SECRET` e fail-closed (503 se ausente no env, 401 se inválido, 400 se `storeId` omitido). Bateria de testes offline C01–C10 (`test/fiscal/scenario-battery/fiscal-scenario-battery.test.ts`) 100% verde cobrindo autorização feliz (100), lote assíncrono (103->105->104->100), duplicidade (204) sem consumo de novo número, indisponibilidade SEFAZ (108/109) fail-closed, consulta não constante (217), consumo indevido/throttling (656) com parada imediata de fila e sem retry cego, timeout/estado incerto com reconciliação idempotente, cancelamento fiscal (110111/135) com validação de prazo (24h SP) e assinatura válida, inutilização de número (102) com lock e sem reutilização, e contingência offline (tpEmis=9) com validação integrada de DANFCE HTML e ESC-POS (80mm/58mm), QR-Code v3 assinado offline (SHA-1/RSA) e imutabilidade de payload SHA-256. Auditoria 175 estritamente honrada: política de cStat sem suposições para 203, 208, 215 e 225 (`COVERAGE_GAP_UNMODELED_CSTAT=false`). Relatório técnico de prontidão para G-F7 produzido (`READY_FOR_G_F7_REVIEW=true`), com gate mantido estritamente fechado (`G_F7_AUTO_ACTIVATION=false`). `SP_ONLY=true` · `SEFAZ_REQUEST_COUNT=0` · `SEFAZ_SOAP_POST_COUNT=0` · `NFCE_EMISSION_COUNT=0` · `FISCAL_OFF=true` · `PRODUCTION_REQUIRED=false` · `SCHEMA_CHANGED=false` · `MIGRATION_CREATED=false`. 25/25 testes do GOAL verdes, regressão fiscal 309/309 testes verdes, typecheck zero erros. Detalhe: [`docs/fiscal/FISCAL_021_OBSERVABILITY_BATTERY_REPORT.md`](../fiscal/FISCAL_021_OBSERVABILITY_BATTERY_REPORT.md).**

> **Adendo — 06 Set 2026 · Fiscal · GOAL 020 containment final da activation H9/H10 consumida e fechamento formal de H-9, H-10 e GOAL 020 (`FISCAL-020-H9H10-FINAL-CONTAINMENT-AND-CLOSE-173`):** a activation `wsdl-h9h10-20260906-1520z-eb237492a9c8eb17`, armada no merge `67e11279638586a380f07fa3525fa3392aa30e2d` (PR #167), foi consumida com sucesso absoluto em uma única execução humana autenticada (`ADMIN_CALL_COUNT=1`, HTTP 200, `ok=true`, `code=completed`, `BATCH_TARGET_COUNT=6`). Todos os 6 alvos WSDL responderam HTTP 200 `text/xml; charset=utf-8` (`SEFAZ_NETWORK_GET_SUCCESS=6/6`, `SEFAZ_HTTP_200=6/6`, `TLS_MTLS_REAL_PATH_PASS=true`). A correção canônica multi-op de `NFeAutorizacao4` (`expectedOperationName = "nfeAutorizacaoLote"`, fix 169) foi validada ao vivo em homologação: o contrato foi extraído com sucesso (`h9=true`, `h10=true`, SHA-256 `b1c67b11afb4bdbb54903eee908ad14694b4b9a1cd78518af89c612f79fb3771`, 4058 bytes), assim como os outros 5 serviços (`NFeRetAutorizacao4`, `NFeConsultaProtocolo4`, `NFeStatusServico4`, `NFeInutilizacao4`, `NFeRecepcaoEvento4`), resultando em `H9_PASS=6/6`, `H10_PASS=6/6` e `NFE_AUTORIZACAO4_MULTI_OP_REAL_PASS=true`. Blocker TLS/ICP-Brasil fechado; blocker multi-op NFeAutorizacao4 fechado. A configuração versionada foi restaurada a `{null,null,null}` e a activation consumida foi registrada como histórica e estritamente proibida de reutilização (`CONSUMED=true`, `REUSABLE=false`). Nenhuma operação SOAP foi executada; nenhuma NFC-e foi emitida; Fiscal Production permaneceu OFF (`FISCAL_OFF=true`). `H9_STATUS=FECHADO` · `H10_STATUS=FECHADO` · `GOAL_020_STATUS=COMPLETED` · `GOAL_021_STATUS=NOT_STARTED`. Trust ICP-Brasil v10, SecureContext SEFAZ, A1 mTLS, catálogo dos 6 targets, one-shot global, advisory lock, lease, resolver e guards preservados. Detalhe: [evidência 173](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-FINAL-CONTAINMENT-AND-CLOSE-173.md).**

> **Adendo — 06 Set 2026 · Fiscal · GOAL 020 rearm da janela FINAL H9/H10 (`FISCAL-020-H9H10-FINAL-REARM-172`):** após a publicação do fix canônico multi-op de `NFeAutorizacao4` (`expectedOperationName = "nfeAutorizacaoLote"`, PR #166 mergeado), foi armada a activation inédita `wsdl-h9h10-20260906-1520z-eb237492a9c8eb17`, válida de `2026-09-06T15:20:00.000Z` (12:20 BRT) a `2026-09-06T16:05:00.000Z` (13:05 BRT) (45 min). `ACTIVATION_ID_IS_FRESH=true` · `EXPIRED_ACTIVATION_REUSED=false` · `EXACTLY_6_HOMOLOG_TARGETS=true` · `NFE_AUTORIZACAO4_MULTI_OP_FIX_PRESENT=true`. `FISCAL_OFF=true` · `H9_STATUS=PARCIAL_5_DE_6` · `H10_STATUS=PARCIAL_5_DE_6` · `GOAL_020=RUNNING` · `GOAL_021=NOT_STARTED`. Nenhuma chamada administrativa, GET WSDL, handshake, SOAP, emissão NFC-e ou consumo de activation executado pelo agente. Trust ICP-Brasil v10, SecureContext, A1 mTLS, catálogo dos 6 targets, GET-only, HOMOLOGACAO-only, one-shot global, advisory lock, lease, resolver e guards preservados. Detalhe: [evidência 172](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-FINAL-REARM-172.md).**

> **Adendo — 05 Set 2026 · Fiscal · GOAL 020 suporte canônico multi-operação NFeAutorizacao4 (`FISCAL-020-NFEAUTORIZACAO4-MULTIOP-EXTRACTION-FIX-169`):** o blocker de extração de `NFeAutorizacao4` (`extraction:operacao_ambigua`), comprovado na execução real de H9/H10 (#168), foi corrigido de forma estrita e sem heurística. O extrator `extractSefazWsdlContract` passou a resolver bindings SOAP 1.2 com múltiplas operações exclusivamente quando o contexto canônico do alvo declarar a operação esperada (`expectedOperationName`). O catálogo de alvos (`wsdl-acquisition-target.ts`) projeta explicitamente `expectedOperationName = "nfeAutorizacaoLote"` para `NFeAutorizacao4`, comprovada no MOC 7.00 Visão Geral e na especificação oficial do leiaute 4.00, diferenciando-a do método compactado `nfeAutorizacaoLoteZip`. A seleção é por igualdade estrita (`attrOf(op, "name").trim() === alvo.expectedOperationName`); zero match e duplicidade falham fechados (`operacao_ausente` / `operacao_ambigua`); serviços sem metadata explícita continuam recusando multi-operação (`operacao_ambigua`). `soapAction` continua lida verbatim da operação selecionada e wrappers são derivados sem convenção. Regressão zero nos 5 serviços e compatibilidade total com o batch efêmero comprovadas offline. Nenhuma chamada de rede, abertura de janela, execução SOAP ou emissão NFC-e realizada. `FISCAL_OFF=true` · `GOAL_020=RUNNING` · `GOAL_021=NOT_STARTED`.**

> **Adendo — 05 Set 2026 · Fiscal · GOAL 020 containment da activation H9/H10 consumida (`FISCAL-020-H9H10-CONSUMED-WINDOW-CONTAINMENT-EVIDENCE-168`):** a activation `wsdl-h9h10-20260905-1516z-025c3251e20744df`, armada no merge `2e589a1ae9a33e168537edcc8a7767891a62889f` (PR #164), foi consumida uma única vez em execução humana autenticada. A invocação administrativa retornou HTTP 200 com `ok=false` e `code=completed`; os 6 alvos retornaram HTTP 200 `text/xml; charset=utf-8`, com `SEFAZ_NETWORK_GET_SUCCESS=6/6`, `SEFAZ_HTTP_200=6/6` e `TLS_MTLS_REAL_PATH_PASS=true`. H-9 e H-10 ficaram `PARCIAL_5_DE_6`: NFeAutorizacao4 teve transporte bem-sucedido, mas o extractor recusou o binding com mais de uma `wsdl:operation`, classificado exclusivamente como `extraction:operacao_ambigua`; o blocker TLS anterior não se reproduziu. A janela foi restaurada a `null/null/null` e a activation ficou histórica e proibida de reutilização. `FISCAL_OFF=true` · `H9_STATUS=PARCIAL_5_DE_6` · `H10_STATUS=PARCIAL_5_DE_6` · `GOAL_020=RUNNING` · `GOAL_021=NOT_STARTED`; trust ICP-Brasil v10, SecureContext, A1 mTLS, catálogo de 6 targets, GET-only/HOMOLOGACAO-only, one-shot, advisory lock, lease, resolver e guards preservados. Detalhe: [evidência 168](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-CONSUMED-WINDOW-CONTAINMENT-168.md).**

> **Adendo — 05 Set 2026 · Fiscal · GOAL 020 rearm de janela H9/H10 inédita (`FISCAL-020-H9H10-REARM-FRESH-WINDOW-166`):** após o containment OFF do PR #163, foi armada a activation nova `wsdl-h9h10-20260905-1516z-025c3251e20744df`, válida de `2026-09-05T15:16:00.000Z` a `2026-09-05T16:01:00.000Z` (45 min). `ACTIVATION_ID_IS_FRESH=true` · `EXPIRED_ACTIVATION_REUSED=false` · `EXACTLY_6_HOMOLOG_TARGETS=true`. `FISCAL_OFF=true` · `H9_STATUS=ABERTO` · `H10_STATUS=ABERTO` · `GOAL_020=RUNNING` · `GOAL_021=NOT_STARTED`; nenhuma execução humana ou externa foi feita pelo agente. Trust ICP-Brasil v10, SecureContext, A1 mTLS, catálogo, GET-only, HOMOLOGACAO-only, one-shot, advisory lock, lease, resolver e guards preservados. Detalhe: [evidência 166](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-REARM-FRESH-WINDOW-166.md).**

> **Adendo — 05 Set 2026 · Fiscal · GOAL 020 containment OFF da segunda janela H9/H10 expirada (`FISCAL-020-H9H10-SECOND-EXPIRED-WINDOW-CONTAINMENT-OFF-165`):** a activation `wsdl-h9h10-20260904-2325z-fcad5be0637f918c` (23:25→00:10Z) expirou. A evidência rastreável disponível confirma o arming/merge #161, mas não prova execução; portanto `ADMIN_CALL_COUNT=UNKNOWN_OR_ZERO_BY_EVIDENCE` e `EXTERNAL_WSDL_GET_COUNT=UNKNOWN_OR_ZERO_BY_EVIDENCE`, sem inferir consumo. A configuração versionada foi restaurada a `{null,null,null}` e a activation foi registrada como histórica/morta e não reutilizável. Trust ICP-Brasil v10, catálogo dos 6 WSDLs, one-shot, lease, resolver da loja-piloto, guards H9/H10, HOMOLOGACAO-only e GET-only preservados. `FISCAL_OFF=true` · `H9_STATUS=ABERTO` · `H10_STATUS=ABERTO` · `GOAL_020=RUNNING` · `GOAL_021=NOT_STARTED`. Detalhe: [evidência 165](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-SECOND-EXPIRED-WINDOW-CONTAINMENT-165.md).**

> Última atualização: 05 Set 2026 — **PDV · Receber conta multitítulo — UI G3 (`PDV-RECEBIMENTO-MULTITITULO-UI-G3-005`):** o `PdvRecebimentoModal` deixou de ser um título por vez. Depois do cliente, abas **Em aberto / Recebidos**, checkbox por título, **Selecionar todos**, valor parcial por título e CTA `Receber N títulos — R$ X` gravando **numa única requisição** em `POST /api/pdv/receber-conta-lote` (endpoint do G2, consumido sem alteração). **"Quitar este título" preservado** na rota singular. O casamento cliente × título por substring bidirecional (achado H: "Ana" casava com "Mariana"/"Joana") foi substituído por escada determinística `clienteId → documento → telefone → nome exato`, decisiva no primeiro degrau com valor dos dois lados — sem identificação segura o título não aparece; com o corretivo P1 de homônimos (EXACT_NAME_HOMONYM_SAFE=true · AMBIGUOUS_NAME_FAILS_CLOSED=true · CROSS_CUSTOMER_RECEIPT_BLOCKED=true), o fallback por nome exato exige unicidade no catálogo de clientes e vendas PDV a prazo passam a registrar clienteId (PDV_APRAZO_PERSISTS_CLIENT_ID=true). `clienteId` passou a ser exposto (aditivo) por `/api/ops/contas-receber-list`. Total confirmado é o `totalRecebido` do servidor; `idempotencyKey` por tentativa econômica, reusada no retry incerto e encerrada só em desfecho definitivo; 409 `saldo_divergente`/`titulo_alterado`/`idempotency_conflict`/`caixa_fechado` tratados com recarga, limpeza de seleção stale e **zero retry automático**; duplo submit travado de forma síncrona. **Recibo consolidado** (um cupom para N títulos) com nome real da loja resolvido de `useLojaAtiva()` — `"Minha Loja"` deixou de ser alcançável; cupom singular intacto. Regra financeira extraída para módulos puros (`lib/contas-receber-lote.ts`, `lib/contas-receber-cliente-match.ts`) porque o harness do Vitest é `node` e não compila `.tsx`. `SCHEMA_CHANGED=false` · `G2_BACKEND_REGRESSION=false` · 45 testes focados + 200 de regressão · **PR aberta, NÃO mergeada**, aguardando revisão independente por outra família. Detalhe: [`docs/audits/RELATORIO_PDV_RECEBIMENTO_MULTITITULO_UI_G3_005.md`](../audits/RELATORIO_PDV_RECEBIMENTO_MULTITITULO_UI_G3_005.md). Anterior — 04 Set 2026 — **Fiscal · GOAL 020 containment OFF da janela H-9/H-10 19:55z expirada SEM CONSUMO (`FISCAL-020-H9H10-EXPIRED-WINDOW-CONTAINMENT-OFF-163`):** a janela `wsdl-h9h10-20260904-1955z-d2c844a079986c9e` (arming 19:55→20:40Z de 04/09, PR #159, deploy ON `dpl_oEcLAXP1XvwRiBj7QNFYrD7iSwLx` / `0728915437f8`) **expirou sem consumo** dentro do prazo. Nenhuma chamada administrativa foi realizada (`WSDL_ADMIN_CALL_COUNT_FOR_ACTIVATION=0`, `WSDL_EXTERNAL_GET_COUNT_FOR_ACTIVATION=0`, `SEFAZ_SOAP_POST_COUNT=0`, `SEFAZ_PRODUCTION_REQUEST_COUNT=0`, `RAW_WSDL_PERSISTED=false`). Janela versionada restaurada imediatamente a `{null,null,null}`. Activation registrada como histórica/morta. Trust ICP-Brasil v10 mantida intacta. `H9_STATUS=ABERTO` · `H10_STATUS=ABERTO` · `GOAL_020=RUNNING` · `GOAL_021=NOT_STARTED` · `PRODUCTION_FISCAL=OFF` · `WSDL_WINDOW=null/null/null`. Anterior — 03 Set 2026 — **Fiscal · GOAL 020 correção da trust anchor ICP-Brasil v10 (`FISCAL-020-SEFAZ-ICPBRASIL-V10-SCOPED-TRUST-160`):** `ROOTCAUSE=TLS_TRUST_ANCHOR_ICPBRASIL_V10_GAP_CONFIRMED` · `TRUST_FIX_IMPLEMENTED=SCOPED_SEFAZ_SECURE_CONTEXT` · `TRUST_ROOT_SHA256=6E:0B:FF:06:9A:26:99:4C:15:DE:2C:48:88:CC:54:AF:84:88:2E:54:95:B7:FB:F6:6B:E9:CC:FF:EC:74:89:F6` · `H9_STATUS=ABERTO` · `H10_STATUS=ABERTO` · `GOAL_020=RUNNING` · `PRODUCTION_FISCAL=OFF` · `WSDL_WINDOW=null/null/null`. A AC Raiz ICP-Brasil v10 (`lib/fiscal/provider/sefaz/trust/icp-brasil-v10.pem`) foi integrada exclusivamente ao SecureContext do transporte SEFAZ (`sefaz-runtime-ports.ts`), preservando integralmente todas as 144 raízes padrão do Node.js, com tracing configurado em `next.config.mjs`, validação estática de integridade criptográfica e sem alterar trust global do processo. Anterior — 03 Set 2026 — **Fiscal · GOAL 020 diagnóstico H-9/H-10 EXECUTADO — falha TLS classificada + containment OFF (`FISCAL-NFCE-CONTINGENCY-020-H9H10-TLS-DIAGNOSTIC-CONTAINMENT-154`):** a activation `wsdl-h9h10-20260903-0037z-b3913bea58774deb` (arming 00:37→01:22Z, ON `2c78d1f` → PR #153 → `main = b09449f`, deploy ON `dpl_AoHpnZjH` / `r0dj2h12b`) foi **CONSUMIDA** — diferente das janelas 139/140/142/144/147/150, esta **não** expirou vazia. UMA invocation administrativa same-origin, browser-assisted, ADMIN/Production: `WSDL_ADMIN_CALL_COUNT=1` · `HTTP_ADMIN_STATUS=200` · `code="completed"` · `ok=false` · `BATCH_TARGET_COUNT=6`. Os **6** alvos canônicos (NFeAutorizacao4, NFeRetAutorizacao4, NFeConsultaProtocolo4, NFeStatusServico4, NFeInutilizacao4, NFeRecepcaoEvento4) falharam de forma **idêntica e pré-HTTP**: `TRANSPORT_PHASE=SECURE_CONNECT` · `DIAGNOSTIC_ROOT_CLASS=TLS_CERTIFICATE` · `DIAGNOSTIC_ROOT_CODE=UNABLE_TO_GET_ISSUER_CERT_LOCALLY` (`failureClass=acquisition:wsdl_rede_incerta`; `httpStatus`/`byteLength`/`sha256` nulos; `h9=false`; `h10=false`). `HTTP_RESPONSE_FROM_WSDL_TARGETS_REACHED=false` · `RAW_WSDL_RECEIVED=false` · `RAW_WSDL_PERSISTED=false` · `SEFAZ_SOAP_POST_COUNT=0` · `SEFAZ_PRODUCTION_REQUEST_COUNT=0`. A telemetria 138 converteu o `wsdl_rede_incerta` opaco da execução 137 em diagnóstico tipado por fase/classe/código. **`H9_STATUS=ABERTO` · `H10_STATUS=ABERTO`**; a distinção entre **cadeia incompleta apresentada pela SEFAZ** e **trust store Node/Vercel insuficiente** **NÃO** está decidida e pertence ao próximo GOAL (nada foi alterado em CA/trust store, TLS, hostname/IP ou destino). Containment OFF commit `bc66b6b`: janela versionada de volta a `{null,null,null}` e a activation consumida acrescentada às proibidas — **one-shot GASTO, jamais re-materializável** (dupla trava: também expirada por relógio às 01:22Z). Preservados sem alteração: ledger one-shot, advisory lock, dedupe global entre lojas, teto de 6 alvos, permit único por alvo, gate resiliente 152, telemetria 138, GET-only, zero retry, `rejectUnauthorized=true`, TLS ≥ 1.2, destinos canônicos HOMOLOGACAO/SP, A1 por refs opacas, auth ADMIN/Production-only, provider e schema. Acumulado honesto do GOAL 020: **3** chamadas administrativas — 134 (HTTP 409 no guard, 0 alvos), 137 e 154 (HTTP 200, 6 alvos cada) —, **0** WSDL válidas. Classificação **C-EXTERNAL-TLS-CHAIN**; GOAL 020 RUNNING · GOAL 021 não iniciado · `track close` não executado. Publicação do OFF: commits `bc66b6b` + `894bb48` + `3bc0fef` → **PR #154** (revisão independente REJECT P0=0 P1=2 → correções → **APPROVE P0=0 P1=0**; checks 6/6 verdes) → mergeada 02:37:20Z, **`main = 2e28093`** → deploy Production OFF `dpl_BBhVQcV4` (`4vqryvrvh`) **READY** 02:42:20Z → alias canônico confirmado no OFF por `vercel inspect` **e** por `GET /api/version` (`buildId=2e28093c35c0`) → ON `r0dj2h12b` (`dpl_AoHpnZjH`) + preview `9skp20s0f` **removidos** ⇒ **`ACTIVE_ON_DEPLOYMENTS_REMAINING=0`** no projeto canônico (os snapshots do projeto irmão `omni-gestao` não são superfície ON — recusados por host **e** por `VERCEL_PROJECT_ID`; ver nota de escopo na [evidência 154](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-TLS-DIAGNOSTIC-CONTAINMENT-154.md)). Anterior — 02 Set 2026 — **Fiscal · GOAL 020 gate H-9/H-10 resiliente à latência de deploy (`FISCAL-NFCE-CONTINGENCY-020-H9H10-DEPLOY-LATENCY-RESILIENT-GATE-152`):** a janela versionada permanece **DORMENTE** `{null,null,null}`. Elegibilidade externa (arming, teto 45 min) foi separada da lease interna de rede (`min(consumedAt + 10 min, expiresAtUtc)`); permits e `wsdlExecutionActivationStillActive` revalidam só a lease. Sem 2 min seguros até o deadline externo, o consumo falha antes de persistir e antes de qualquer socket. One-shot, 6 alvos, GET-only, TLS, destinos, A1 e superfície Production-only intocados. ZERO SEFAZ · ZERO activation real · ZERO Production ON. `READY_FOR_H9H10_RESILIENT_GATE=true`. GOAL 020 RUNNING · GOAL 021 não iniciado. Detalhe: [evidência 152](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-DEPLOY-LATENCY-RESILIENT-GATE-152.md). Anterior — 02 Set 2026 — **Fiscal · GOAL 020 janela H-9/H-10 21:27z NÃO UTILIZADA (`FISCAL-NFCE-CONTINGENCY-020-H9H10-UNUSED-WINDOW-CONTAINMENT-150`):** janela `wsdl-h9h10-20260902-2127z-bfefedc2de8f65f9` (21:27→21:37Z / 18:27→18:37 BRT) foi materializada (ON `fc28c25` → PR #149 → `main = fe50630`, deploy ON `dpl_CGPPBZNG` / `r7vu7jg1b` ainda Building em 21:29:06Z com 7,9 min restantes) e o gate ≥ 8 min FALHOU. Fetch NÃO entregue; invocation administrativa NÃO realizada. Contadores LOCAIS desta janela: `WSDL_ADMIN_CALL_COUNT=0` · `WSDL_EXTERNAL_GET_COUNT=0` · `SEFAZ_SOAP_POST_COUNT=0` · `SEFAZ_PRODUCTION_REQUEST_COUNT=0` · `RAW_WSDL_PERSISTED=false` (o GOAL 020 acumulado NÃO tem zero rede: evidência 137 registrou 1 chamada administrativa HTTP 200 e batch dos 6 alvos). One-shot íntegro · activation proibida. Containment OFF: `bd8850b` → PR #150 (revisão independente APPROVE P0=0 P1=0) → `main = 453566c` → deploy OFF `dpl_8pzh5yfN` (`2qe1vllq4`) READY com alias → ON `r7vu7jg1b` + preview `epx5c26x5` removidos (0 restantes). Telemetria 138 segue em produção. Classificação **B-EXTERNAL-TIMING**; GOAL 020 RUNNING · GOAL 021 não iniciado; próxima tentativa exige NOVA autorização textual. Detalhe: [evidência 150](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-UNUSED-WINDOW-CONTAINMENT-150.md). Anterior — 02 Set 2026 — **Fiscal · GOAL 020 janela H-9/H-10 14:00z expirou SEM CONSUMO (`FISCAL-NFCE-CONTINGENCY-020-H9H10-EXPIRED-WINDOW-CONTAINMENT-147`):** janela `wsdl-h9h10-20260902-1400z-4b5f2504640de6e4` (14:00→14:10Z / 11:00→11:10 BRT) foi materializada (ON `e0f0477` → PR #146 → `main = 3d18984`, deploy ON `dpl_AQ6vtihX` / `mgrnjlga4` READY com alias, fetch same-origin entregue com ~31 min restantes) e a invocation administrativa NÃO foi realizada. Contadores LOCAIS desta janela: `WSDL_ADMIN_CALL_COUNT=0` · `WSDL_EXTERNAL_GET_COUNT=0` · `SEFAZ_SOAP_POST_COUNT=0` · `SEFAZ_PRODUCTION_REQUEST_COUNT=0` · `RAW_WSDL_PERSISTED=false` (o GOAL 020 acumulado NÃO tem zero rede: evidência 137 registrou 1 chamada administrativa HTTP 200 e batch dos 6 alvos). One-shot íntegro · activation proibida. Containment OFF: `d78c27f` → PR #147 → `main = f3d6ece` → deploy OFF `dpl_5UsyPShA` (`hvhyxpw00`) READY com alias → ON `mgrnjlga4` + preview `lauumehec` removidos (0 restantes). Telemetria 138 segue em produção. Classificação **B-EXTERNAL-TIMING**; GOAL 020 RUNNING · GOAL 021 não iniciado; próxima tentativa exige NOVA autorização textual. Detalhe: [evidência 147](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-EXPIRED-WINDOW-CONTAINMENT-147.md). Anterior — 02 Set 2026 — **Fiscal · GOAL 020 janela H-9/H-10 04:30z expirou SEM CONSUMO (`FISCAL-NFCE-CONTINGENCY-020-H9H10-EXPIRED-WINDOW-CONTAINMENT-144`):** janela `wsdl-h9h10-20260902-0430z-772103b09d9477ca` (04:30→04:40Z) foi materializada (ON `56254ee` → PR #143 → `main = 5be0b79`, deploy ON `dpl_HivW5dv` / `1rf5laffe` READY com alias canônico, fetch same-origin entregue) e a invocation administrativa NÃO foi realizada. Contadores LOCAIS desta janela: `WSDL_ADMIN_CALL_COUNT=0` · `WSDL_EXTERNAL_GET_COUNT=0` · `SEFAZ_SOAP_POST_COUNT=0` · `SEFAZ_PRODUCTION_REQUEST_COUNT=0` · `RAW_WSDL_PERSISTED=false` (o GOAL 020 acumulado NÃO tem zero rede: evidência 137 registrou 1 chamada administrativa HTTP 200 e batch dos 6 alvos). One-shot íntegro · activation proibida. Containment OFF: `d313c99` → PR #144 (revisão independente APROVADO P0=0 P1=0) → `main = 5a0e70f` → deploy OFF `dpl_5tDHksyq` (`8ydcds7fg`) READY com alias → ON `1rf5laffe` + preview `dk04mh82b` removidos (0 restantes). Telemetria 138 segue em produção. Classificação **B-EXTERNAL-TIMING**; GOAL 020 RUNNING · GOAL 021 não iniciado; próxima tentativa exige NOVA autorização textual. Detalhe: [evidência 144](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-EXPIRED-WINDOW-CONTAINMENT-144.md). Anterior — 02 Set 2026 — **Fiscal · GOAL 020 janela H-9/H-10 22:30z expirou SEM CONSUMO (`FISCAL-NFCE-CONTINGENCY-020-H9H10-EXPIRED-WINDOW-CONTAINMENT-142`):** janela `wsdl-h9h10-20260831-2230z-891f55e242004bd2` (22:30→22:40Z) foi materializada (ON `7d8f61c` → PR #140 → `main = facf2be`, deploy ON `dpl_Gg6XLgEo` / `8qh22op1x` READY com alias canônico) e a invocation administrativa NÃO foi realizada (nenhum fetch manual). Contadores LOCAIS desta janela: `WSDL_ADMIN_CALL_COUNT=0` · `WSDL_EXTERNAL_GET_COUNT=0` · `SEFAZ_SOAP_POST_COUNT=0` · `SEFAZ_PRODUCTION_REQUEST_COUNT=0` · `RAW_WSDL_PERSISTED=false` (o GOAL 020 acumulado NÃO tem zero rede: evidência 137 registrou 1 chamada administrativa HTTP 200 e batch dos 6 alvos). One-shot íntegro · activation proibida. Containment OFF: `3c2d724` → PR #141 → `main = 5ebc8a7` → deploy OFF `dpl_6H5JuYCq` (`bhd7n154x`) READY com alias → ON `8qh22op1x` + preview `iqp0y09po` removidos (0 restantes). Telemetria 138 segue em produção. Classificação **B-EXTERNAL-TIMING**; GOAL 020 RUNNING · GOAL 021 não iniciado; próxima tentativa exige NOVA autorização textual. Detalhe: [evidência 142](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-EXPIRED-WINDOW-CONTAINMENT-142.md). Anterior — 31 Ago 2026 — **Fiscal · GOAL 020 diagnóstico browser-assisted expirou SEM CONSUMO (`FISCAL-NFCE-CONTINGENCY-020-DIAG-EXPIRED-NO-CONSUMPTION-140`):** janela `wsdl-h9h10-20260831-1900z-99c21bca85a94cef` (19:00→19:10Z, gate textual vigente) foi materializada (ON `6137773` → PR #137 → `main = 5777356`, deploy ON `dpl_2NRViE3` READY com alias canônico, revisão independente P0=0 P1=0) e o comando JavaScript same-origin foi entregue ao humano — que não o executou a tempo (relógio 19:34:18Z). `WSDL_ADMIN_CALL_COUNT=0` · `WSDL_EXTERNAL_GET_COUNT=0` · one-shot íntegro · activation proibida. Containment OFF: `0e6fe53` → PR #138 → `main = 3bc7e05` → deploy OFF `dpl_HqPwz84y` READY com alias → ON `ripbdw99x` + preview `738goze7r` removidos (0 restantes). Telemetria 138 segue em produção. Classificação **B-EXTERNAL-TIMING**; próxima tentativa exige NOVA autorização textual (nenhuma anterior é reutilizável). Detalhe: [evidência 140](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-DIAG-EXPIRED-NO-CONSUMPTION-140.md). Anterior — 31 Ago 2026 — **Fiscal · GOAL 020 diagnóstico H-9/H-10 BLOQUEADO por credencial ADMIN (`FISCAL-NFCE-CONTINGENCY-020-DIAG-LOGIN-BLOCKED-139`):** janela efêmera `wsdl-h9h10-20260831-0300z-0c42c4389f65469d` (03:00→03:10Z, gate textual verbatim) foi materializada (ON `c0d6810` → PR #135 → `main = 285b459`, deploy ON READY), mas o login ADMIN pré-janela falhou com `CredentialsSignin` (`loja-1` e `ADMIN_DEFAULT_PASSWORD` de produção rejeitados) e a ÚNICA chamada NÃO foi executada — fail-closed. `WSDL_ADMIN_CALL_COUNT=0` · zero GETs · activation NÃO consumida (one-shot íntegro, morta por relógio) · nenhum segredo impresso · segredos temporários apagados. Containment OFF completo: commit `8afa132` → PR #136 → `main = a546ca9` → deploy OFF `dpl_6ySRdQDKS` READY com alias canônico → ON production `8qdg5jf8b` + 2 previews ON removidos (0 restantes). Telemetria 138 segue em produção, pronta para a próxima janela. Classificação **B-EXTERNAL-DEPENDENCY**; próximo passo exige credencial ADMIN válida + NOVO gate textual (a autorização não é reutilizável). Detalhe: [evidência 139](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-DIAG-LOGIN-BLOCKED-139.md). Anterior — 30 Ago 2026 — **Fiscal · GOAL 020 telemetria sanitizada de transporte WSDL (`FISCAL-020-H9H10-TRANSPORT-TELEMETRY-138`):** falhas que antes viravam apenas `acquisition:wsdl_rede_incerta` (evidência 137) agora carregam telemetria tipada e sanitizada `transportPhase`/`transportClass`/`transportCode` por desfecho de aquisição e por serviço no batch. Classe derivada EXCLUSIVAMENTE de allow-list fechada de códigos Node (ENOTFOUND/EAI_AGAIN→DNS; ECONNREFUSED/EHOSTUNREACH/ENETUNREACH→TCP_CONNECT; ECONNRESET/EPIPE→CONNECTION_RESET; ETIMEDOUT→TIMEOUT; 6 códigos de cadeia/identidade→TLS_CERTIFICATE; EPROTO+5 TLS→TLS_HANDSHAKE); fora da lista ⇒ `UNKNOWN`/`UNKNOWN_NETWORK`. Fase observada (REQUEST_CREATE/BEFORE_SECURE_CONNECT/SECURE_CONNECT/RESPONSE_STREAM/REQUEST_LIFECYCLE), nunca inferida — ausência de secureConnect não vira "TLS". Timeouts `wsdl_timeout_conexao`/`wsdl_deadline_total` preservados. Nenhuma mensagem/stack/cause/syscall/address/hostname/IP/cert/senha atravessa (testes com erros envenenados); integrações loopback reais provam ECONNREFUSED, reset, cadeia CA divergente, handshake não-TLS e corte pós-resposta. Revisão independente APROVADO P0=0 P1=0 (7/7 focos); 386 testes pass; typecheck/lint/build/diff-check OK; PR #134 mergeado — `main = 71e1ca4`, deploy Production `dpl_7Er9hEX…` READY com alias canônico. ZERO rede no GOAL (`WSDL_EXTERNAL_GET_COUNT=0` · `SEFAZ_REQUEST_COUNT=0` · `SEFAZ_SOAP_POST_COUNT=0`); janela `null/null/null`; deployments ON seguem removidos; `track check` PASSOU sem close; GOAL 021 não iniciado. **READY_FOR_DIAGNOSTIC_HUMAN_GATE=true** — parado aguardando autorização textual específica (a anterior não é reutilizável). Detalhe: [`docs/ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-TRANSPORT-TELEMETRY-138.md`](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-TRANSPORT-TELEMETRY-138.md). Anterior — 29 Ago 2026 — **Fiscal · GOAL 020 refresh do gate H-9/H-10 (`FISCAL-020-H9H10-GATE-REFRESH-129`):** gate WSDL efêmero atualizado ao estado atual do projeto, integralmente DORMENTE e sem rede (`WSDL_EXTERNAL_GET_COUNT=0` · `SEFAZ_REQUEST_COUNT=0`). Literal de loja-piloto `"loja-1"` REMOVIDO: piloto resolvida dinamicamente de `ConfiguracaoFiscalLoja` (HOMOLOGACAO + NFCE + SEFAZ_DIRETO + certificado ativo; exatamente UMA candidata; zero/múltiplas/erro → fail-closed; request deve pertencer exatamente à candidata). One-shot GLOBAL preservado sem schema: `pg_advisory_xact_lock` + verificação cross-store na transação de consumo (unique `(storeId,dedupeKey)` como retaguarda). Janela restaurada a `null/null/null` (ativação 24/08 é evidência histórica). Regra `fiscalEnabled` decidida e documentada: preflight exige `fiscalEnabled=true` (a piloto é a loja operacional do pipeline 020 em HOMOLOGACAO; o `false` legado era proxy de fase sem pipeline) — sem habilitar emissão (superfície é GET de metadados, sem corpo/SOAP, alvos fechados do catálogo). Superfície canônica Production-only, 6 GETs máx, TLS ≥ 1.2, zero redirect, A1 por refs opacas preservados. Sem schema, sem SEFAZ, sem merge neste GOAL (PR aguarda revisão). Detalhe: [`docs/ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-GATE-REFRESH-129.md`](../ai-execution/_evidence/FISCAL-NFCE-CONTINGENCY-020-H9H10-GATE-REFRESH-129.md). Anterior — 29 Ago 2026 — **PDV · Venda em Espera nos três PDVs:** o contrato local existente de `lib/pdv-hold` foi mantido e passou a atualizar a UI após salvar/remover holds, com filtro por tipo de PDV. Assistência, Clássico e Supermercado exibem acesso visível `Em espera`, preservam cliente, descontos, acessórios (modelo/cor) e metadados de linha compatíveis, protegem o carrinho atual com confirmação e permitem múltiplos holds isolados por loja/terminal. Hold continua sem criar venda nem movimentar estoque, Financeiro, Caixa ou Fiscal. GOAL `PDV-VENDA-EM-ESPERA-RESTORE-001`.. Anterior — 27 Ago 2026 — **Fiscal · GOAL 019 inutilização NFC-e RECONCILIADA sobre a main pós-018 (`FISCAL-NFCE-INVALIDATION-019`):** os 7 commits do 019 foram reaplicados semanticamente sobre `ce1b993` (main com GOAL 018 DONE), substituindo a PR #114 divergida. Número consumido nunca volta ao pool: job `INUTILIZACAO` idempotente integrado ao worker, marca `A_INUTILIZAR` baixada só com `cStat 102`+protocolo, rejeitada enfileira inutilização, `SIMULATED_CAN_AUTHORIZE=false`, XMLDSig estrutural do `inutNFe` (bytes assinados == bytes no SOAP), CAS da marca, `NUMBER_REUSE_COUNT=0`. Cancelamento 018 integralmente preservado. Sem schema, **sem merge neste GOAL** (PR nova aguarda decisão humana), sem SEFAZ ao vivo. `G_C7_STATUS=FECHAVEL_INTERNO` · `HOMOLOGATION_STATUS=internal-only`. Detalhe: [`docs/fiscal/FISCAL_019_INUTILIZACAO_NFCE_INVALIDATION_REPORT.md`](../fiscal/FISCAL_019_INUTILIZACAO_NFCE_INVALIDATION_REPORT.md). Anterior — 25 Ago 2026 — **Fiscal · GOAL 018 cancelamento NFC-e (`FISCAL-NFCE-CANCELLATION-018`):** evento SEFAZ `NFeRecepcaoEvento4` idempotente `(notaFiscalId, tipo, sequencia)`, prazo SP 30 min (Portaria SRE 40/2024 + Ajuste SINIEF 19/16; CAT 12/2015 revogada), persistência fail-closed somente com resposta **não simulada** e `cStat 135` (evento registrado; `101` só na consulta), composição `SefazDiretoProvider`+`SefazSoapTransport`+A1 (sem fallback stub, sem REGISTRY P1), transições `NotaFiscal.AUTORIZADA→CANCELADA` e `Venda→CANCELADA_FISCAL`, rota `POST /api/fiscal/notas/[id]/cancelar`, **zero escrita Financeiro/Caixa**. Sem schema, sem produção, sem inutilização (GOAL 019). Homologação SEFAZ real depende do gate H-9/H-10 (expirado neste corte). Anterior — 21 Ago 2026 — **Contador HUB · DASHBOARD INTERNO HONESTO (`CONTADOR-HUB-INTERNAL-REALIFICATION-023`):** nenhuma fixture do HUB interno afirma mais estado da empresa do usuário. Saíram funcionários fictícios, escritório contábil de exemplo, razão social/CNPJ de exemplo, resumo financeiro e KPIs fabricados, progresso "3 de 9 / 35%", situação inventada por documento nos dossiês e o Radar CNPJ que dizia "certidão vencida" sobre um CNPJ nunca consultado. O aviso global stale — que negava envio, fechamento, guia e documento enquanto Documentos/Fechamento/Obrigações já eram reais — foi **removido**; o preview restante é identificado **no bloco**. **Folha & DP** virou a projeção dos documentos reais de categoria `folha` da competência (sem cálculo de folha, holerite, encargos, eSocial, ponto ou cadastro de colaborador; `RH_ENGINE_CREATED=false`), **Portal do contador** ganhou resumo real de acessos/convites pelos contratos do GOAL 014, **Configurações** lê o cadastro persistido da loja (`Store.name`/`cnpj`) e esconde o que não tem persistência, e **Dossiês** virou híbrida (itens `sistema` com correspondência inequívoca leem o DTO do GOAL 006). **Zero CTA no-op** sobrou. Exportação individual por relatório **não** foi implementada (exigiria nova superfície de API/auth; duplicar CSV é vedado) — o bloco aponta o arquivo equivalente dentro do Pacote. `SCHEMA_CHANGED=false` · `EXTERNAL_PORTAL_CHANGED=false` · `R2_CHANGED=false`. Anterior — 21 Ago 2026 — **Contador HUB · PORTAL V2 OPERACIONAL EM PRODUCTION:** storage real em Cloudflare R2 (bucket privado dedicado + CORS restrito ao domínio de Production), portal v2 **on**, portal legado **off**, `CONTADOR_RETENCAO_APPLY` **ausente**. Smoke autenticado real executado ponta a ponta com identidade de smoke dedicada (papel LEITURA, uma loja, **revogada ao final**): convite → aceite → sessão → escopo → competências → **escrita e leitura reais no R2 com SHA-256 conferido** → cross-store negado (403 sem vazamento) → isolamento de cookie nos dois sentidos → logout com invalidação server-side → revogação de sessão ativa → soft delete com trilha preservada. Pacote oficial **não disponível** (a v1 nasce no primeiro fechamento; fechar só para o teste é vedado). Corretivo de UI mergeado antes do smoke (PR **#104**, `ea1eaf6`): link de convite deixou de ser inalcançável quando o clipboard do navegador está bloqueado. `CONTADOR_PRODUCTION_SMOKE_PASS=true` · `CONTADOR_OPERATIONALLY_LIVE=true` · `PRODUCTION_RETENTION_APPLY_EXECUTED=false` · `SCHEMA_CHANGED=false`. Evidência: [`docs/contador/OPERACAO_CONTADOR_019.md`](../contador/OPERACAO_CONTADOR_019.md) §12. Anterior — 20 Ago 2026 — **Contador HUB · GOAL 018 FECHADO NA MAIN (`CONTADOR-HUB-FISCAL-INTEGRATION-018`):** integração fiscal read-only mergeada em `main` (PR **#97**, merge commit `870ed54`). Reader **A** + homologação **B**, flag `CONTADOR_FISCAL_READER` default off + allowlist env-only obrigatória, `dhEmi` fail-closed sem fallback. `FISCAL_RUNTIME_VALIDATABLE=true` · `PRODUCTION_XML_ELIGIBLE=false` · `SEFAZ_NETWORK_USED=false` · `SCHEMA_CHANGED=false`. AEP: GOAL 018 **DONE** · trilha `contador` **PAUSED** · `current_goal=null` · `GOAL_019_OPENED=false`. Anterior — **Fiscal · GOAL 030 PIX dinâmico YA04 (`FISCAL-030-PIX17-YA04-NONINTEGRATED-089`):** novas emissões NFC-e com PIX dinâmico (tPag 17) emitem `<card><tpIntegra>2</tpIntegra></card>` com evidência server-side. `tpIntegra=1`, ausente/inválido e legado 17 sem evidência são fail-closed. Estático 20 / automático 23 permanecem sem card. PaymentModal/Caixa/Financeiro intactos. Sem schema, sem SEFAZ. Classificação **A**. Detalhe: [`docs/fiscal/FISCAL_PAYMENT_PIX17_YA04_NONINTEGRATED_089_REPORT.md`](../fiscal/FISCAL_PAYMENT_PIX17_YA04_NONINTEGRATED_089_REPORT.md) · ADR-0023 (**aceita/vigente**, adendo 089). Anterior — **Fiscal · GOAL 030 POS não integrado (`FISCAL-030-CARD-POS-SIMPLE-TPINTEGRA-087`):** novas emissões NFC-e com cartão 03/04 emitem `<card><tpIntegra>2</tpIntegra></card>` com evidência server-side. `tpIntegra=1`, CNPJ/`tBand`/`cAut` e legado sem evidência são fail-closed. PaymentModal/Caixa/Financeiro intactos. PIX 17+YA04 permanece residual. Sem schema, sem SEFAZ. Classificação **A**. Detalhe: [`docs/fiscal/FISCAL_PAYMENT_CARD_POS_SIMPLE_TPINTEGRA_087_REPORT.md`](../fiscal/FISCAL_PAYMENT_CARD_POS_SIMPLE_TPINTEGRA_087_REPORT.md) · ADR-0023 (**aceita/vigente**, adendo 087). Anterior — **Operações V4 · Closeout dos blockers (`OPS-V4-BLOCKERS-FINAL-CLOSEOUT-018`):** V4 passou de **parcialmente implementada** para **operacionalmente fechada**. Os 7 blockers restantes (A-G) foram resolvidos: (A) persistência real de `OSStatusChange`/`OSTimelineEvent` via Server Actions; (B) `OsListItem.tecnico` agora usa `tecnicoNome` persistido; (C) `OSTimeline` e `OSKanban` leem eventos persistidos; (D) `OSDocuments` usa a API real de documentos; (E) `OSPrint` usa dados reais da OS; (F) `OSWhatsApp` envia via API real; (G) `OSNew` cria OS persistida. V4 é agora o motor operacional de produção. Anterior — **Fiscal · GOAL 030 PIX tPag 17 (`FISCAL-030-PIX-TPAG-17-DYNAMIC-085`):** novas emissões NFC-e com PIX dinâmico emitem `tPag=17` (sem YA04). PIX estático permanece `20`; PIX automático permanece `23`. Legado PIX sem evidência de tipo continua fail-closed. Caixa/Financeiro intactos. Sem schema, sem SEFAZ. Classificação **A**. Detalhe: [`docs/fiscal/FISCAL_PAYMENT_PIX_TPAG_17_DYNAMIC_085_REPORT.md`](../fiscal/FISCAL_PAYMENT_PIX_TPAG_17_DYNAMIC_085_REPORT.md). Anterior — **Fiscal · GOAL 030 PIX automático (`FISCAL-030-PIX-AUTOMATIC-TPAG-23-083`):** novas emissões NFC-e com PIX automático emitem `tPag=23`. PIX estático permanece `20`. Sem schema, sem SEFAZ. PIX dinâmico (`tPag=17`) permanece residual. Classificação **A**. Anterior — **Operações V4 no dashboard (`OPS-V4-DASHBOARD-SWAP-017`):** `/dashboard/operacoes` agora monta a V4 (fonte de verdade persistida). A V2 isolada permanece em `/dashboard/operacoes-v2` como legado. Sem persistência nova, sem schema, sem Server Actions novas. Classificação **A**. Anterior — **Contador HUB · GOAL 018 implementação fiscal read-only (`CONTADOR-HUB-FISCAL-INTEGRATION-018`):** reader **A** (`lib/contador/readers/fiscal.ts` SELECT Prisma, sem FiscalLog) + homologação **B**. Flag `CONTADOR_FISCAL_READER` default off + allowlist obrigatória env-only `CONTADOR_FISCAL_READER_STORE_ALLOWLIST`. Predicado ADR-007 com `dhEmi` fail-closed (corretivo, sem fallback). `FISCAL_RUNTIME_VALIDATABLE=true` · `PRODUCTION_XML_ELIGIBLE=false`. Revisão independente **A**. PR **#97** **MERGEADO** em `main` (merge commit `870ed54`). GOAL 018 **DONE** (`GOAL_018_CLOSED=true`). Anterior — **Fiscal · GOAL 030 PIX (`FISCAL-030-PIX-TPAG-20-082`):** novas emissões NFC-e com PIX emitem `tPag=20` (sem YA04). Caixa/Financeiro intactos. Sem schema, sem SEFAZ. PIX dinâmico (`tPag=17`) permanece residual. Classificação **A**. Anterior — **Operações V4 (`OPS-V4-OS-ENGINE-016`):** motor persistido da OS (Server Actions + `lib/operacoes/v4/`) implementado e validado. UI V4 (`components/operacoes/v4/`) pronta mas **ainda não** no dashboard — `/dashboard/operacoes` continua V2. Sem schema. Classificação **A**. Anterior — **Fiscal · GOAL 030 numerário (`FISCAL-030-NUMERARIO-TROCO-081`):** novas emissões NFC-e em dinheiro (`tPag=01`) com `cashTendered` válido passam a emitir `<vTroco>` no XML. Caixa/Financeiro intactos. Sem schema, sem SEFAZ. Classificação **A**. Anterior — **WhatsApp HUB (`WHATSAPP-REAL-PERSISTENCE-079`):** Inbox/Chat/Templates/Automations/Quick Replies/Broadcast **reais** (`storeId` + Prisma). Relatórios/Configurações **stub explícito**. Sem Meta Graph, sem schema. Classificação **A**. Anterior — **Fiscal · GOAL 030 crédito/vale (`FISCAL-030-CREDITO-LOJA-TPAG-21-080`):** novas emissões NFC-e com crédito da loja / vale-crédito passam a emitir `tPag=21`. Caixa/Financeiro intactos. Sem schema, sem SEFAZ. Classificação **A**. Anterior — **Fiscal · GOAL 030 carnê/a prazo (`FISCAL-030-CARNE-APRAZO-FAILCLOSED-077`):** carnê e a prazo **bloqueados** na emissão NFC-e (`PAGAMENTO_CARNE_NAO_SUPORTADO` / `PAGAMENTO_APRAZO_NAO_SUPORTADO`). Caixa, Financeiro, OS e estoque intactos. Sem schema, sem SEFAZ. Classificação **A**. Anterior — **Fiscal · GOAL 030 contrato de pagamento (`FISCAL-030-PAYMENT-CONTRACT-075`):** contrato canônico de pagamento NFC-e documentado e testado. Emissão continua fail-closed para meios sem evidência. Classificação **A**. Anterior — **Estoque (`ESTOQUE-UI-INTEGRATION-074`):** Estoque HUB UI integrada ao backend persistido (`/dashboard/estoque`). Sem schema. Classificação **A**.

> Referência rápida para retomar o projeto ou fazer onboarding.

> **Entrada obrigatória de novos chats/agentes:** [`docs/ai/START_HERE.md`](./START_HERE.md) —
> leitura obrigatória + política do Neon centralizada lá (não duplicada aqui).

---

## Contador HUB — DASHBOARD INTERNO HONESTO · 21/08/2026

```
SECTIONS_TOTAL=11
REAL_COUNT_AFTER=8
HYBRID_COUNT_AFTER=3
PREVIEW_COUNT_AFTER=0
GLOBAL_NOTICE_STALE_REMOVED=true
FECHAMENTO_PREVIEW_PILL_REMOVED=true
DOCUMENT_BADGE_REAL_OR_REMOVED=removed
FOLHA_FAKE_EMPLOYEES_REMOVED=true
FOLHA_DOCUMENTOS_REAL=true
RH_ENGINE_CREATED=false
PORTAL_FAKE_OFFICE_REMOVED=true
PORTAL_REAL_SUMMARY=true
CONFIG_FAKE_COMPANY_REMOVED=true
CONFIG_PERSISTENCE_CREATED=false
DOSSIES_SYSTEM_REALIFIED=true
DOSSIES_PREVIEW_REMAINDER_HONEST=true
REPORT_EXPORT_REALIFIED=false
DEAD_FIXTURES_REMOVED=7
SCHEMA_CHANGED=false
EXTERNAL_PORTAL_CHANGED=false
R2_CHANGED=false
```

**Classificação das 11 seções.** REAL (8): Fechamento, Documentos, Obrigações, Permissões
& acesso, Timeline, Folha & DP, Portal do contador, Configurações. HÍBRIDA (3): Visão geral,
Relatórios, Dossiês — todas com o bloco preview identificado localmente. PREVIEW total: nenhuma.

**O que era enganoso e saiu.** `FOLHA_FUNCIONARIOS` (Ana Souza / Carlos Lima / Marina Reis),
pró-labore com nome de sócio e holerite ilustrativo; o cartão "Escritório Contábil Exemplo ·
acesso somente leitura · ativo" do Portal; "Loja Exemplo Ltda" / "00.000.000/0001-00" nas
Configurações; `RESUMO_FINANCEIRO` (R$ 48,2k de vendas etc.) renderizado **ao lado** do resumo
REAL da mesma competência; `VISAO_KPIS`; `VISAO_DOSSIE_PROGRESS` e o anel "3 de 9 / 35%";
o `status` fabricado por linha de dossiê; o Radar CNPJ com "CND Federal válida" / "Alvará
vencido" / "2 pendências e-CAC"; o badge numérico `4` da aba Documentos; e o `GlobalPreviewNotice`,
cujo texto afirmava «Nenhum envio, fechamento, guia ou documento é processado» — falso desde os
GOALs 010/012/016. Fixtures mortas apagadas (grep = zero consumo produtivo): `VISAO_ALERTAS`,
`OBRIGACOES_ROWS`, `FECHAMENTO_CHECKLIST`, `DOCUMENTOS_ROWS`, `PERMISSOES_ROWS`, `MESES`,
`COMPETENCIA_INICIAL`.

**O que virou real.** **Folha & DP** (`components/dashboard/contador/folha/`) lista os documentos
de categoria `folha` da competência via `GET /api/contador/documentos?c=…&categoria=folha`, com
download autorizado por URL assinada, estados de carga/vazio/erro e microcopy explícita sobre o
que fica fora (cálculo de folha, holerite, encargos, eSocial, ponto, cadastro de colaborador).
**Portal do contador** (`components/dashboard/contador/portal/`) resume o acesso externo pelos
mesmos contratos da aba Permissões (`/api/contador-externo/acessos` + `/convites`): ativos,
convites pendentes, suspensos, revogados, papel e data — com 403 dito como "não é seu para ver",
nunca como "nenhum contador". A lista «o contador pode» foi corrigida: o portal v2 **não** recebe
upload. **Configurações** exibe `Store.name`/`Store.cnpj` da loja ativa por um reader server-side
escopado (`lib/contador/readers/loja.ts`) — sem endpoint novo, sem schema novo — e **oculta** os
controles sem persistência. **Dossiês** virou híbrida: itens de origem `sistema` com correspondência
inequívoca no DTO do GOAL 006 (fluxo realizado, contas a pagar, contas a receber, formas de
pagamento) mostram o valor real **com a disponibilidade que o próprio DTO declara**; os demais
...
**Pendência declarada.** `REPORT_EXPORT_REALIFIED=false`: gerar o CSV individual de um relatório
reusando o gerador do Pacote exige nova superfície de API/auth sobre o endpoint endurecido
`GET /api/contador/pacote` (hoje só devolve o ZIP inteiro), e duplicar a lógica de CSV é vedado.
O bloco assume isso e aponta o arquivo equivalente dentro do pacote; "Posição de estoque" fica
marcado como **sem fonte** (o DTO do Contador não lê Estoque).

---

## Contador HUB — PORTAL V2 OPERACIONAL EM PRODUCTION · 21/08/2026

```
CONTADOR_PORTAL_V2_PRODUCTION_ACTIVE=true
CONTADOR_LEGACY_DISABLED=true
CONTADOR_INFRA_PRODUCTION_READY=true
R2_PRODUCTION_READY=true
R2_BUCKET_PRIVATE=true
R2_CORS_RESTRITO_A_PRODUCTION=true
R2_RUNTIME_WRITE_PASS=true
R2_RUNTIME_READ_PASS=true
SMOKE_DOWNLOAD_BYTES_MATCH=true
SMOKE_DOWNLOAD_HASH_MATCH=true
AUTHORIZED_STORE_PASS=true
CROSS_STORE_BLOCK_PASS=true
COOKIE_ISOLATION_PASS=true
LOGOUT_PASS=true
REVOCATION_PASS=true
ACTIVE_ACCESS_AFTER_SMOKE=false
SMOKE_DOCUMENT_SOFT_DELETED=true
AUDIT_TRAIL_PRESERVED=true
PACKAGE_DOWNLOAD_PASS=not_available
CONTADOR_PRODUCTION_SMOKE_PASS=true
CONTADOR_OPERATIONALLY_LIVE=true
CONTADOR_RETENCAO_APPLY=ausente
PRODUCTION_RETENTION_APPLY_EXECUTED=false
SCHEMA_CHANGED=false
```

**O que mudou de fato.** O Contador saiu de "infra provisionada" para **operacional**: o
portal externo v2 está no ar, o legado desligado, e o storage real em Cloudflare R2 foi
exercitado por escrita e leitura verdadeiras — não por checker. Um documento sintético foi
enviado pelo fluxo real do produto e baixado pelo portal externo com **bytes e SHA-256
idênticos**; a autorização de download devolve apenas URL assinada, sem expor `storageRef`.

**Achado que faltava no provisionamento.** Bucket R2 nasce **sem CORS**, e o upload do
Contador é direto do navegador para a URL assinada. Sem a regra de CORS o envio quebrava
como erro de rede genérico. Nenhum smoke sem sessão detecta isso — as rotas externas param
em 401 antes de tocar o storage. A regra aplicada é restrita ao domínio de Production.

**Segurança verificada em produção.** Loja não concedida responde 403 genérico, sem vazar
nome, competência, documento ou pacote. Isolamento de cookie provado nos dois sentidos:
sessão de contador não vira sessão de ERP, e sessão de ERP comprovadamente válida não vira
sessão de contador. Logout invalida no servidor (cookie reusado → 401), e revogação do
vínculo derruba sessão **ativa** na requisição seguinte. Soft delete preserva a trilha:
`documento_enviado` → `documento_download_autorizado` (ator externo) → `documento_excluido`.

**Pendências.** Pacote oficial ainda não existe — a v1 nasce no primeiro fechamento, e
fechar competência só para teste é vedado. Contadores ainda não foram comunicados da nova
URL. Existe um objeto órfão no bucket (upload cujo `complete` foi recusado), inerte e sem
registro no acervo, que a retenção desligada não remove. Detalhe operacional em
[`docs/contador/OPERACAO_CONTADOR_019.md`](../contador/OPERACAO_CONTADOR_019.md) §12.
...
SCHEMA_CHANGED=false
CONTADOR_HUB_FISCAL_INTEGRATION_018=DONE
GOAL_018_CLOSED=true
GOAL_019_OPENED=false
CONTADOR_FISCAL_READER=off (default; liga só com valor exato "on")
CONTADOR_FISCAL_READER_STORE_ALLOWLIST=env-only CSV (sem schema)
```

Reader A em `lib/contador/readers/fiscal.ts` (SELECT `NotaFiscal` isolado por `storeId`; nunca `fiscalXmlReader.readAuthorizedDocument`; nunca grava FiscalLog/EventoFiscal). Checklist atualiza só o sinal fiscal. Pacote substitui `05-XML/LEIA-ME.md` apenas com XML entregável (texto UTF-8 persistido + sha256). UI: relatório fiscal mínimo somente leitura. Runtime homolog local (Postgres nativo 5432 nesta prova): 1 AUTORIZADA/HOMOLOGACAO entregável, 1 rejeitada, 1 cancelada; FiscalLog=0; EventoFiscal=0. Corretivo `dhEmi` fail-closed (competência só do XML autorizado, sem fallback). Revisão independente **A** (aprovada: `dhEmi` fail-closed, reader A, homologação B). PR **#97** **MERGEADO** em `main` — merge commit `870ed54841f42f42dbabe37268c3dd6e9e6708e7`. AEP: GOAL 018 **DONE** · trilha `contador` **PAUSED** · `current_goal=null` · `last_goal=CONTADOR-HUB-FISCAL-INTEGRATION-018` · `last_result=DONE`. GOAL 019 **não aberto** (`GOAL_019_OPENED=false`) — planejamento humano. Detalhe: [`CONTADOR_HUB_FISCAL_HOMOLOGATION_PROVISION_018.md`](../contador/CONTADOR_HUB_FISCAL_HOMOLOGATION_PROVISION_018.md).

---

## Contador HUB — Passo 0 fiscal FECHADO + ADR-007 Accepted · 19/08/2026

Auditoria [`CONTADOR_HUB_FISCAL_PASSO0_AUDIT_018.md`](../contador/CONTADOR_HUB_FISCAL_PASSO0_AUDIT_018.md) na main (PR **#88** merge `fac38130`). ADR-CONTADOR-007 **Accepted** (PR **#89** MERGED) com predicado Opção A **e** `ambiente == HOMOLOGACAO` (`ENTREGAVEL_AMBIENTE=HOMOLOGACAO_ONLY_INITIAL_ROLLOUT`; `PRODUCTION_XML_ELIGIBLE=false`), cancelados fora de `05-XML`, competência por `dhEmi` de `xmlAutorizado` **sem fallback**, flag `CONTADOR_FISCAL_READER` default off + allowlist de loja (não `fiscalEnabled`; allowlist **não** substitui o gate de ambiente), homologação isolada (nunca Production nesta fase; Production no entregável exige decisão humana posterior) e **proibição** de reusar `fiscalXmlReader.readAuthorizedDocument` as-is (`FISCAL_READER_AS_IS_FORBIDDEN=true`). Reader **A** e homologação **B** **ratificados** por Rafael (2026-08-20). Preparação: [`CONTADOR_HUB_FISCAL_HOMOLOGATION_PREP_018.md`](../contador/CONTADOR_HUB_FISCAL_HOMOLOGATION_PREP_018.md). Auditoria de prontidão: [`CONTADOR_HUB_FISCAL_HOMOLOGATION_READINESS_AUDIT_018.md`](../contador/CONTADOR_HUB_FISCAL_HOMOLOGATION_READINESS_AUDIT_018.md). Provisionamento B: [`CONTADOR_HUB_FISCAL_HOMOLOGATION_PROVISION_018.md`](../contador/CONTADOR_HUB_FISCAL_HOMOLOGATION_PROVISION_018.md). `FISCAL_RUNTIME_VALIDATABLE=false` (estado esperado **antes** do GOAL 018; é critério de **aceite**, não de import/open). `PRE_OPEN_NON_PRODUCTION_RUNTIME_REQUIRED=true`. `NON_PRODUCTION_RUNTIME_AVAILABLE=true` com Postgres local dedicado + seed HOMOLOGACAO. `GOAL_018_OPENED=true` (esta entrega). `GOAL_018_STATUS=RUNNING`. Trilha AEP `contador` **RUNNING** no GOAL 018. `current_goal=CONTADOR-HUB-FISCAL-INTEGRATION-018`.

---

## Contador HUB — Homologação B provisionada (sem GOAL 018) · 20/08/2026

Rafael autorizou merge do PR **#92** e o Postgres **efêmero/local** com seed HOMOLOGACAO. Reader = Opção **A** (`lib/contador/readers/fiscal.ts` SELECT read-only — **implementado no GOAL 018**). Homologação = Opção **B** (Prisma + Postgres local + 7 rows persistidas). DSN dedicado: host loopback, database `omni_contador_fiscal_homolog`, role `omni_homolog`, portas `54329`/`5432` — recusa db/role de dev e remoto. Sem Production, sem SEFAZ, sem `prisma/schema.prisma`. Massa em `homolog-contador-a` / `homolog-contador-b`. `PRE_OPEN_NON_PRODUCTION_RUNTIME_REQUIRED=true` · `NON_PRODUCTION_RUNTIME_AVAILABLE=true` · `FISCAL_RUNTIME_VALIDATABLE=true` (prova 018: Prisma → reader A → predicado → checklist → 05-XML). Detalhe: [`CONTADOR_HUB_FISCAL_HOMOLOGATION_PROVISION_018.md`](../contador/CONTADOR_HUB_FISCAL_HOMOLOGATION_PROVISION_018.md).

---

## Contador HUB — GOAL 017 FECHADO NA MAIN (PR #85) · 19/08/2026

Central de avisos **REAL** na Visão geral. Sem botão Enviar. Canal classificação **B**. Contrato: [`docs/contador/OMNI_AGENT_CONTRATO_017.md`](../contador/OMNI_AGENT_CONTRATO_017.md). AEP: GOAL 017 **DONE** · trilha `contador` **PAUSED** · `current_goal=null`. `GOAL_018_OPENED=false` · `GOAL_018_STATUS=DRAFT_NOT_IMPORTED`.
...
- [`CONTADOR_HUB_ADRS_PROPOSTOS_001.md`](../contador/CONTADOR_HUB_ADRS_PROPOSTOS_001.md) — 8 ADRs (001/003/004/005/006 Accepted G2; 002/008 Accepted G3; **007 Accepted 2026-08-19**; `GOAL_018_OPENED=false` · `GOAL_018_STATUS=DRAFT_NOT_IMPORTED`).
...
>
> **PR #8** merge `b307337…` (GOAL-004). **PR #12** merge
> `2a7f102ce7bb22b363cd6d24b17920d483182640` (GOAL-005A; parents `98e05df…` + `d512794…`;
> `merged_at` `2026-07-19T15:08:52Z`). `origin/main` contém `2a7f102…`.
...
### GOAL-004 — paridade fiscal do `upsertProduto` (Cadastros V2)

- **Estado:** **FECHADO** — implementação + merge commit + fechamento documental.
- **Equivalência histórica:** GOAL histórico **002 / P-04** (paridade `upsertProduto`); parcial
  prévio `04ce54d` (contrato + REST/importadores). **Não** renumerar docs históricos.
- **Arquivos (4):** `app/actions/cadastros.ts` · `produto-ia.tsx` ·
  `lib/produtos/produto-fiscal-upsert.ts` · `lib/produtos/produto-fiscal-upsert.test.ts`
  (+206 / −20).
- **`metadata.fiscal`:** **única fonte fiscal canônica** do Produto (10 campos do contrato
  `lib/produto-fiscal.ts` — **inalterado**): ncm, cest, cfop, cst, csosn, origemMercadoria,
  unidadeComercial, unidadeTributavel, codigoAnp, exTipi.
- **`metadata.fiscalRegime`:** namespace de **compatibilidade visual/textual** do Cadastros V2
  (`tributacao`, `origem`, `atualizadoEm`) — **não** canônico; **não** lido por
  `getProdutoFiscal`, motor Fiscal, APIs fiscais ou importadores; **não** autoriza emissão.
- **Comportamento:** create canônico; update sem fiscal preserva bloco; update parcial
  não-destrutivo; metadata não fiscal preservada; campos desconhecidos rejeitados; sem default
  tributário falso.
- **Barcode/Cosmos:** sugestões NCM/CEST sob **revisão humana** (não auto-confirmadas).
- **Schema / migration / regra tributária:** não alterados · nenhuma · nenhuma.
- **Nível N:** **N3** neste eixo cadastro; **N6=0**; **N7=0** (sem operação real / SEFAZ /
  homologação / produção).
- **Gates:** nenhum avançado por este GOAL (G-C1/G-C2/C14N inalterados; F4→F5 global, G-F5,
  G-F7, G-F12 abertos).
- **Checks do PR #8:** Vercel **success**; **sem** checks independentes de TypeScript, ESLint,
  testes unitários ou build no GitHub — evidências **reutilizadas da auditoria** de merge
  readiness (61/61 testes; TS/ESLint/build verdes).
- **Follow-ups (não implementados):** R-1 legado `fiscal.tributacao` → `fiscalRegime`; R-2
  exibição NCM/CEST via `getProdutoFiscal`; R-3 UI demais campos fiscais; R-4 status de
...
### GOAL-005A — supply chain GHA do worker XSD (fechado na main) · 19/07/2026

- **Estado:** **integrado e fechado na main** (PR **#12** merged).
- **Merge:** `2a7f102ce7bb22b363cd6d24b17920d483182640` · head `d512794…` · branch origem
  preservada · método merge commit.
...
- **Riscos remanescentes:** dry-run global incompleto; ST/CSOSN 500; provider real; fila/eventos;
  G-F5/G-F7/G-F12; follow-ups R-1…R-5 do GOAL-004.
- **Documentos:** [roadmap](../roadmaps/ROADMAP_FISCAL.md) ·
  ...
  [fechamento GOAL-004](../fiscal/FISCAL_PRODUTO_UPSERT_PARITY_004_CLOSURE_REPORT.md) ·
...
## Operações V4 — Closeout dos blockers (`OPS-V4-BLOCKERS-FINAL-CLOSEOUT-018`) · 20/08/2026

> Fecha os gaps reais restantes da V4 reusando motores já existentes.
> Sem schema novo, sem segundo PDV, sem PIN/auth paralelo.

- **Execução / Estoque:** `consumirEstoqueOSActionV3` → adapter `consumeEstoqueFromOS`. Replay = `already_consumed`. Timeline + livro-razão (`origem=os`, documento da OS). A V4 recarrega na hora.
- **Rail Receber:** deixa de parecer um PDV operacional read-only. Lista abre o Financeiro/Recebimento real da OS (`openOSFromRail(..., true)`). Sem segundo motor de pagamento.
- **Técnicos:** seletor passa a unir `listTecnicos` (cadastro da loja) com os já atribuídos nas OS.
- **Config / Segurança:** Config da V4 e a superfície residual de segurança abrem `/dashboard/configuracoes`. Sem PIN paralelo.
- **Auditoria de UX:** sai da navegação operacional.

---

## Operações V4 — Dashboard / Histórico / acabamento (`OPS-V4-DASHBOARD-HISTORICO-FINAL-017`) · 19/08/2026

> Fecha a Visão geral e o Histórico transversal da OS com dados já existentes.
> Sem schema, sem motor paralelo.

- **Visão geral (`DashboardV4`):** fila por status, SLA, sem técnico/bancada, prontas, retornos, entregas do dia — cliques abrem a rail ou a OS.
- **Histórico:** timeline filtrável, cliente/aparelho, OS relacionadas (aparelho/cliente/retorno/origem), anexos, assinatura, observação interna real, export de auditoria, ligar via `tel:`.
- **Acabamento:** selo "Protótipo" sai das rails restantes; ligar/export/histórico do cliente deixam de ser no-op.

---

## Operações V4 — Técnico / Bancada / Fila / SLA (`OPS-V4-TECNICO-BANCADA-FILA-016`) · 19/08/2026

> Fecha o fluxo operacional de chão de oficina na V4. O motor continua 100% V3
> (payload-only). Sem schema, sem action V4 paralela.

### O que mudou

- **Técnico:** atribuir/trocar/remover na Bancada, Fila, header e Execução via `atribuirTecnicoV3`.
- **Bancada física:** entrar/sair grava `aberturaV3.recepcao.localFisico` (`definirLocalFisicoV3`) + timeline.
- **Status:** Fila (Kanban/Lista) e Bancada seguem a política compartilhada → `aplicarTransicaoStatusV3`.
- **SLA rail própria (`SlaV4`):** agrupa atrasadas / em risco / no prazo / sem prazo com `lerSlaV3` (não mais `os.sla.status` legado isolado).
- **Observação interna e checklist técnico:** `adicionarObservacaoInternaV3` e `salvarChecklistTecnicoV3` (sem `updateOSPayload`).
- **Identidade das rails:** Fila, Bancada e SLA abrem o Workspace na Execução (`openOSProducao`). CTA comercial da Bancada continua abrindo o cockpit sem override.

**Segurança:** sem schema/migration, sem tocar PDV/Caixa/Financeiro engine/Estoque/WhatsApp/Fiscal/auth/proxy.

---

## Operações V4 — Documentos, assinatura e anexos (`OPS-V4-DOCUMENTOS-ASSINATURA-ANEXOS-015`) · 19/08/2026

> Fecha a camada documental da OS na V4 reusando os motores V3 já existentes.
> Sem schema, sem action paralela, sem Cloud API nova.

### O que mudou

- **Impressão / reimpressão:** o menu Docs e a etapa Entrega abrem `PrintPreviewV3` para OS cliente, Termo de Garantia, Termo de Entrega, via interna, etiqueta e orçamento. `registrarImpressaoDocumentoV3` continua auditando na timeline.
- **Assinatura:** retirada digital segue em `entregaV3.assinaturaRetirada` (`SignaturePadV3` + `salvarAssinaturaRetiradaV3`).
- **Fotos de saída:** `entregaV3.fotosSaida` com add/remove (`adicionarFotoSaidaV3` / `removerFotoSaidaV3`); confirmar entrega **preserva** fotos já gravadas.
- **Anexos:** thumbs reais de entrada + saída no rail, histórico e painel de anexos. `+` navega para captura real (Entrada ou Entrega).
- **WhatsApp:** `montarLinkWaV4` / `montarMensagemDocumentoV4` — `wa.me`, só documento visível ao cliente. Sem `/api/whatsapp/send` (inbox exige `conversationId`).

**Segurança:** sem schema/migration, sem tocar PDV/Caixa/Financeiro engine/Estoque/Fiscal/auth/proxy. WhatsApp HUB intocado.

---

## Operações V4 — Guard comercial da Bancada (`OPS-V4-BANCADA-COMMERCIAL-TRANSITION-GUARD-003B`) · 15/08/2026

> Micro-GOAL sobre o commit da Fila: Fila e Bancada passam a consultar a **mesma**
> política de transições rápidas. A máquina V3 continua autoridade.

### O que mudou

- **Policy compartilhada** `lib/operacoes-v4/transicoes-producao-v4.ts`: `proximasTransicoesV3` ∩ destinos de chão − pares comerciais − cancelar/receber/entregar.
- **Bancada:** Diagnóstico e Aguardando aprovação perdem o botão de status. CTAs abrem a OS (`openOSFromRail`). Ações seguras (iniciar serviço, aguardar peça, peça chegou, marcar pronta) continuam.
- **Fila:** continua usando a mesma policy (drag / Mover para). Sem segundo grafo.

**Segurança:** sem schema, sem action nova, sem tocar `enviarOrcamentoV3` / `aprovarOrcamentoV3` / Financeiro / Entrega.

---

## Operações V4 — Fila Kanban acionável (`OPS-V4-FILA-KANBAN-WRITE-004`) · 15/08/2026

> A Fila V4 deixa de ser uma lista somente leitura e vira a esteira operacional
> da assistência. O motor continua 100% V3.

### O que mudou

- **Tela dedicada `FilaV4`:** Kanban + Lista, tickets densos (técnico, prioridade, SLA, defeito).
- **Write real:** `moverStatusFila` → `destinosPermitidosFilaV4` (`proximasTransicoesV3` ∩ recorte de produção) → `aplicarTransicaoStatusV3`. Refresh sem F5 via `runWriteOnOs`.
- **Drag seguro:** só destinos de chão de oficina. Destino inválido não recebe drop. Sem optimistic move — loading por card; erro recarrega a verdade do servidor.
- **Comercial protegido:** diagnóstico→aguardando aprovação e aguardando aprovação→aprovada ficam no cockpit (`enviarOrcamentoV3` / `aprovarOrcamentoV3`).
- **Mesma mudança sem drag:** menu `Mover para`, tecla `M`, Lista.
- **Filtros:** busca, técnico, prioridade, SLA. Preferência Kanban/Lista em `omnigestao:operacoes-v4:fila-view:v1`.
- **Fora deste slice:** recebimento, entrega, cancelamento por drag, pós-venda. `recebida` não entra no Kanban de produção.

**Segurança:** sem schema/migration, sem action V4 paralela, sem segunda máquina de status, sem tocar PDV/Caixa/Financeiro engine/Estoque/WhatsApp/Fiscal/auth/proxy.

---

## Operações V4 — Bancada operacional (`OPS-V4-PRODUCAO-TECNICO-BANCADA-003`) · 14/08/2026

> A Bancada V4 deixa de ser uma rail somente leitura e vira a superfície de
> produção do dia. O motor continua 100% V3.

### O que mudou

- **Tela dedicada `BancadaV4`:** resumo operacional, fila **Sem técnico** (P0), produção agrupada por técnico.
- **Mutations reais:** `atribuirTecnicoV3` (inclui remoção com `null`), `definirPrioridadeV3`, `aplicarTransicaoStatusV3` só nos destinos de chão de oficina autorizados por `podeTransicionarV3`.
- **Adapter puro** `lib/operacoes-v4/producao-v4.ts`: projeção, ordenação (SLA crítico → prioridade → idade), filtros client-side. Sem KPI inventada (sem tempo médio / capacidade / produtividade).
- **Workspace:** tickets de técnico e prioridade no `CommandHeader`; resumo no início da Execução.
- **Fora deste slice (à época):** Kanban drag da Fila (fechado em `OPS-V4-FILA-KANBAN-WRITE-004`), peças/apontamentos write, cadastro de funcionário.

**Segurança:** sem schema/migration, sem action V4 paralela, sem tocar PDV/Caixa/Financeiro engine/Estoque/WhatsApp/Fiscal/auth/proxy. Técnicos conhecidos continuam inferidos das OS (`tecnicosConhecidosV3`) — não há cadastro independente, portanto não se inventam técnicos "livres".

---

## Operações V4 — Pipeline operacional e Entrada simplificada (`OPS-V4-PIPELINE-ENTRADA-NAV-SIMPLIFY-002`) · 14/08/2026

> Segundo GOAL da reorganização operacional da V4. Depois da criação unificada,
> o cockpit deixa de parecer um wizard de oito etapas e passa a refletir o
> fluxo real da assistência.

### O que mudou

- **Pipeline:** só Entrada, Diagnóstico, Execução, Entrega e Pós-venda. Estados visuais concluída / atual / pendente / atenção.
- **Orçamento:** saiu da pipeline. Ticket comercial no header (`Cobrança não definida`, `Orçamento · Rascunho`, `Enviado`, `Aprovado`, `Recusado`) abre o `OrcamentoStage` real.
- **Financeiro:** saiu da pipeline. Ticket financeiro no header (`A receber`, parcial, `Quitado`) abre o `FinanceiroStage` real. Leitura indisponível não se disfarça de ausência de cobrança.
- **Histórico:** saiu da pipeline. Ação `Histórico` + contagem de eventos no header abre o `HistoricoStage` real.
- **Entrada:** 4 grupos — Recepção e aparelho, Segurança e custódia, Inspeção, Evidências. Snapshot do que já veio da abertura. Sem rail overlay `entrada-nav`.
- **Contratos:** helpers puros `pipeline-operacional.ts`, `os-header-transversal.ts`, `identidade-aparelho.ts` e `checklist-aplicabilidade.ts`. Fotos e assinatura da entrada reusam as actions V3. Sem schema, sem motor novo.

**Segurança:** sem schema/migration, sem tocar PDV/Caixa/Financeiro engine/Estoque/WhatsApp/Fiscal/auth/proxy. Destinos `orcamento`/`financeiro`/`historico` continuam válidos; só saíram da trilha.

---

## Operações V4 — Novo atendimento comercial (`OPS-V4-NOVO-ATENDIMENTO-COMERCIAL-001`) · 14/08/2026

> Primeiro GOAL da reorganização operacional da V4. A criação de atendimentos
> deixa de ser três botões no topo e vira uma entrada única e profissional.

### O que mudou

- **TopBar:** um CTA `+ Novo` no lugar de Atendimento rápido / Orçamento rápido / Nova OS.
- **Launcher:** overlay compacto à direita, copy *"Novo atendimento — Escolha como este atendimento começa."* e três tickets:
  - **Nova OS** → `criarOSEnterpriseV3` → workspace Entrada
  - **Orçamento** → `criarOrcamentoRapidoV3` → workspace Orçamento
  - **Atendimento rápido** → `finalizarAtendimentoRapidoV3` → workspace Entrega
- **Empty state** do workspace (`OSPicker`) também entra pelo `+ Novo`.
- **Contrato puro** `lib/operacoes-v4/novo-atendimento.ts` documenta modalidade → motor → destino (preparação para os próximos GOALs de pipeline/recebimento).
- **Formulários:** Nova OS, Novo orçamento e Atendimento rápido passam pelo mesmo shell/accordion e pelas seções compartilhadas de cliente e aparelho.
- **Pré-OS:** `payload.comercialV4` classifica o orçamento; não entra em fila/bancada até converter (mesmo `osId`).
- **Fora / dívida:** catálogo de peças no create ainda é opcional; prova de entrada completa fica no workspace; Instagram/Ligação persistem no carimbo comercial e mapeiam origem V3 para balcão.

**Segurança:** sem schema/migration, sem tocar PDV/Caixa/Financeiro/Estoque/Inventário/WhatsApp/Fiscal/auth/proxy/Vendas HUB/Operações V3 actions.
