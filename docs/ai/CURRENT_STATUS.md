...
Reader A em `lib/contador/readers/fiscal.ts` (SELECT `NotaFiscal` isolado por `storeId`; nunca `fiscalXmlReader.readAuthorizedDocument`; nunca grava FiscalLog/EventoFiscal). Checklist atualiza só o sinal fiscal. Pacote substitui `05-XML/LEIA-ME.md` apenas com XML entregável (texto UTF-8 persistido + sha256). UI: relatório fiscal mínimo somente leitura. Runtime homolog local (Postgres nativo 5432 nesta prova): 1 AUTORIZADA/HOMOLOGACAO entregável, 1 rejeitada, 1 cancelada; FiscalLog=0; EventoFiscal=0. Corretivo `dhEmi` fail-closed (competência só do XML autorizado, sem fallback). Revisão independente **A**. PR **#97** aberto (draft, **não mergeado**). **Não mergear. Não `track close`.** GOAL 018 **RUNNING**, **não fechado**. Detalhe: [`CONTADOR_HUB_FISCAL_HOMOLOGATION_PROVISION_018.md`](../contador/CONTADOR_HUB_FISCAL_HOMOLOGATION_PROVISION_018.md).
