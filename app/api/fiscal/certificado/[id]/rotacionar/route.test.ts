 ...
 *  - CNPJ divergente e fingerprint de outra loja ⇒ 422 e ponteiro intacto;
...
describe("POST rotacionar — recusas fail-closed", () => {
  ...
  it("certificado REVOGADO é terminal ⇒ 409", async () => {
    seedCertAntigo({ status: "REVOGADO", ativo: false })
    const novo = validTestPfx()
    const res = await POST(requestComPfx(Buffer.from(novo.pfx), novo.senha), ctx())
    expect(res.status).toBe(409)
    // Cofre intocado (o antigo segue lá, revogado na linha).
    expect(h.envVault[SENHA_REF]).toBeTruthy()
  })
  ...
  it("CNPJ divergente da unidade ⇒ 422 e ponteiro intacto", async () => {
    seedCertAntigo()
  ...
  it("fingerprint vinculada a OUTRA loja ⇒ 422 e ponteiro intacto", async () => {
    seedCertAntigo()
    const novo = validTestPfx()
  ...
  it("concorrência na troca (guarda otimista) ⇒ 409, linha inalterada, nada órfão", async () => {
    ...
    expect(h.certs[0]!.fingerprint).toBe("ff".repeat(20))
    // EnvVault in-place: refs idênticas ⇒ nada a descartar/revogar (sem autodestruição).
    expect(h.envVault[SENHA_REF]).toBeTruthy()
    void antigo
